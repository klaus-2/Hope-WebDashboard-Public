...
// Want to use or contribute to this? http://github.com/Glitchii/embedbuilder
...
window.onload = () => {
    ...
    let editorHolder = document.querySelector('.editorHolder'),
        ...
        toRGB = (hex, reversed, integer) => {
            ...
            embed: {
                ...
                thumbnail: {
                    url: "https://cdn.discordapp.com/avatars/622812963572809771/596cc27607fc959da17c1f6b75965b9b.png"
                },
                image: {
                    url: `https://images-ext-1.discordapp.net/external/HeyjEAQpQ1O3bEXVh3RZPvWKLncPg6LicPmY4D_JaVE/%3Favatar%3Dhttps%3A%2F%2Fcdn.discordapp.com%2Favatars%2F622812963572809771%2F596cc27607fc959da17c1f6b75965b9b.png%3Fsize%3D2048%26middle%3DWelcome%26name%3DKlaus%25231565%26bottom%3DWe%2520have%2520now%2520174%2520members%26background%3Dhttps%3A%2F%2Fmedia.discordapp.net%2Fattachments%2F908247510991142923%2F908247799429206096%2FBanner.png%26text%3D%2523ffffff%26avatarborder%3D%2523FFFFFF%26avatarbg%3D%2523FF0000/https/api.xzusfin.repl.co/card`
                },
                footer: {
                    text: "Powered by hopebot.top",
                    icon_url: "https://i.imgur.com/HyxPiGq.png"
                },
            }
        };
    ...
    let notif = document.querySelector('.notification'),
        ...
        makeShort = (txt, length, mediaWidth) => {
            ...
            if (re) return error(`URL should start with <code>http://</code> or <code>http://</code> on this line <span class="inline full">${makeShort(re[0], 30, 600)}</span>`);
            if (e.timestamp && new Date(e.timestamp).toString() === "Invalid Date") return error('Timestamp is invalid');
            return true;
        }, markup = (txt, opts) => {
            txt = txt
                .replace(/\&#60;:[^:]+:(\d+)\&#62;/g, '<img class="emoji" src="http://cdn.discordapp.com/emojis/$1.png"/>')
                .replace(/\&#60;a:[^:]+:(\d+)\&#62;/g, '<img class="emoji" src="http://cdn.discordapp.com/emojis/$1.gif"/>')
                .replace(/~~(.+?)~~/g, '<s>$1</s>')
    ...
    document.querySelectorAll('.done .selectOptions .channel').forEach(e => e.addEventListener('click', el => { //
        ...
        if (document.querySelector('.messageContent.empty+.container.empty'))
            return error('Nothing to send.', 2500);
...
if(command.length > 30) return error("Command length exceeds 30 characters.", 4000);
  ...
  error('Successfully saved command, if not saved, <a href="https://hopebot.top/embeds">read here</a>', 10000)
        e.preventDefault();
        $.ajax({
            type: "POST",
